...
module.exports = {
  ...
  paymentCompletedCrossVerification: async (req, res, type, db_connection) => {
    ...
    if (req.paramBody.pg_receipt_id === "") {
      ...
      return await calculateOrderProductsPrice(
        req.paramBody,
        0,
        type,
        db_connection,
      ...
      let pgReceipt = await getBootPaySinglePayment(
...
async function calculateOrderProductsPrice(
  param,
  receiptPrice,
  type,
  db_connection,
  ...
  //선물하기는 주소입력을 안한다.
  if (type !== "gift") {
    await queryCheckIsland(param["addressbook_uid"], db_connection).then(
      (res) => {
        if (res === 1) {
  ...
  switch (type) {
    case "common":
      {
        //일반결제검증함수
        backProductInfo = await querySelectProductInfo(
          frontProductInfo,
          db_connection,
        );
    ...
    case "influencer": {
      //인플루언서 정보를 받고
      backProductInfo = await queryInfluencerProductInfo(
        frontProductInfo,
        db_connection,
      );
      //검증을 여기서 진행한다.
      compareInfluencerInfo(
        frontProductInfo,
        backProductInfo,
        function (compared) {
          objectCalculate["priceTotal"] += compared["priceTotal"];
          objectCalculate["influencer_gongu_uid"] =
            compared["influencer_gongu_uid"];
          objectCalculate["sellerArr"].push(compared);
        },
      );
    }
  ...
  objectCalculate["totalPayment"] =
    objectCalculate["priceTotal"] +
    objectCalculate["totalDelivery"] -
  ...
  console.table(
    [
      {
        pg결제금액: receiptPrice,
        검증결제금액: objectCalculate["totalPayment"],
      },
    ],
    ["pg결제금액", "검증결제금액"],
  );
  ...
  //   throw await sendError(
  //     `결제금액과 검증금액이 맞지 않습니다. 결제를 취소합니다.`,
  //   );
  // }
}
...
async function querySelectProductInfo(frontProductList, db_connection) {
  ...
  return await Promise.all(
    frontProductList.map(async (product_list) => {
    ...
    db_connection.query(queryAddress, (err, rows, field) => {
      if (err) {
        ...
        const queryIsland = `select if(count(uid) > 0, ${true}, ${false}) as island
                                     from tbl_island_area_v2 where zipcode = ${
                                       rows[0]["zipcode"]
                                     }`;
...
function compareProductInfo(
  ...
  if (frontProductInfo.length !== backProductInfo.length) {
    throw sendError(
      `결제할 상품 종류가 일치하지 않습니다. 클라이언트:${frontProductInfo.length}개, 서버:${backProductInfo.length}개`,
    );
  }
  ...
  for (let i = 0; i < backProductInfo.length; i++) {
    ...
    const log = {};
    log.product_uid = new ConsoleValidate(
      fElem["product_uid"],
      bElem["product_uid"],
    );
    log.seller_uid = new ConsoleValidate(
      fElem["seller_uid"],
      bElem["seller_uid"],
    );
    log.priceTotal = new ConsoleValidate(
      fElem["price_original"] * fElem["count"],
      (bElem["price_discount"] + bElem["option_price"]) * fElem["count"],
    );
    ...
    if (
      bElem["sale_type"] === "soldout" ||
      bElem["count_total"] === bElem["count_sale"]
    ) {
    ...
    if (fElem["product_uid"] !== bElem["product_uid"]) {
      ...
      throw sendError(
        `상품번호가 일치하지 않습니다. product_uid: ${fElem["product_uid"]}`,
      );
    }
    if (fElem["seller_uid"] !== bElem["seller_uid"]) {
      // if(fElem['seller_uid'] !== 0){
      throw sendError(
        `판매자번호가 일치하지 않습니다. seller_uid: ${fElem["seller_uid"]}`,
      );
    }
...
function compareInfluencerInfo(
  frontProductInfo,
  backProductInfo,
  calculateCallback,
) {
  ...
  for (let i = 0; i < backProductInfo.length; i++) {
    ...
    const log = {};
    log.price_delivery = new ConsoleValidate(
      fElem["price_delivery"],
      bElem["delivery_price"],
    );
    log.light_delivery_price = new ConsoleValidate(
      fElem["price_delivery"],
      bElem["light_delivery_price"],
    );
    log.seller_uid = new ConsoleValidate(
      fElem["seller_uid"],
      bElem["seller_uid"],
...
function removeAndCalculateDuplicateSellerArr(objectCalculate) {
  ...
  objectCalculate["sellerArr"] = objectCalculate["sellerArr"].reduce(
    (acc, seller) => {
      const existingSeller = acc.find(
        (item) => item.seller_uid === seller.seller_uid,
      );
      if (existingSeller) {
...
async function paymentApproval(objectCalculate) {
  try {
    if (objectCalculate["pg_receipt_id"] !== "") {
      const response = await BootpayV2.confirmPayment(
        objectCalculate["pg_receipt_id"],
      );
