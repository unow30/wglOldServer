...
function getFilename(req, file) {
  try {
    ...
    let hash_name = crypto
      .createHash("md5")
      .update(Date.now() + basename)
      .digest("hex");
    return `${hash_name}${extension}`;
  } catch (e) {
    let _err = errUtil.initError(
      errCode.fail,
      "동영상 변환에 실패했습니다. 다시 시도해주세요.",
    );
...
function uploadFile(req, res, next) {
  ...
  let single = multer(fileOptions).array("files", 1000);
    ...
    if (err) {
      ...
      if (err.code === "LIMIT_FILE_SIZE") {
        sendUtil.sendErrorPacket(
          ...
          errUtil.initError(
            errCode.system,
            `최대 업로드 가능한 파일 사이즈는 ${MAX_LENGTH_MB}mb 입니다.`,
          ),
        );
      } else {
...
function replaceName(filename) {
  ...
  console.log("asdfoksdofsk: " + filename);
