 ...
 *
 * @swagger
 * /api/public/v2/user/signup:
 *   post:
 *     summary: 회원가입
 *     tags: [User]
 *     description: |
 *       path : /api/public/v2/user/signup
 *
 *       * 회원가입
 *       * 해당 api 호출 전 필수 사항
 ...
 */
 ...
 module.exports = function (req, res) {
     ...
     try{
         ...
         mysqlUtil.connectPool( async function (db_connection) {
             ...
             await queryInsertInterest(req, db_connection);
 ...
 async function queryInsertInterest(req, db_connection) {

    console.log('interest insert 시작 ===========>')

    if(req.paramBody['interests'][0] != null){
        const interestData = req.paramBody['interests'].map(result => [req.innerBody['item']['uid'], result]);
